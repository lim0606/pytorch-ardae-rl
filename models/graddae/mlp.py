...
from models.layers import MLP, ResMLP, WNMLP, Identity, ContextScaleMLP
...
class ConditionalARDAE(nn.Module):
    ...
    def forward(self, input, context, std=None, scale=None):
        ...
        ctx = self.ctx_encode(context.view(batch_size*csz, self.context_dim)).view(batch_size, csz, self.ctx_dim).mean(dim=1, keepdim=True)
        ...
        # encode
        _, ctx = expand_tensor(ctx, sample_size=sample_size, do_unsqueeze=False) # bsz*ssz x xdim
        inp = self.inp_encode(x_bar)
        ...
        # return
        return glogprob.view(batch_size, sample_size, self.input_dim).detach(), loss
